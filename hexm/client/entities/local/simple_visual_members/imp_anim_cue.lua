-- ======================================================================
-- Module: hexm.client.entities.local.simple_visual_members.imp_anim_cue
-- Source: package.loaded
-- Type: table
-- Order: #2138
-- ======================================================================

-- Module type: table

SimpleVisualMember: class {
  -- Metatable:
  --   __tostring: yes
  CUE_ANIM_END_HANDLER: <table>
  CUE_USER_DATA_HANDLER: table {
    guishou_touch_npc: function()  -- @hexm/client/entities/local/simple_visual_members/imp_anim_cue.lua:35-39
  }
  __module__: "hexm/client/entities/local/simple_visual_members/imp_anim_cue.lua"
  __post_component__: function(arg1, arg2)  -- @hexm/client/entities/local/simple_visual_members/imp_anim_cue.lua:8-11
  ctor: function(...)  -- =[C]
  handle_npc_anim_end_cue: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/simple_visual_members/imp_anim_cue.lua:13-18
  handle_npc_user_data_cue: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/simple_visual_members/imp_anim_cue.lua:20-25
  new: function(...)  -- =[C]
}


-- End of hexm.client.entities.local.simple_visual_members.imp_anim_cue