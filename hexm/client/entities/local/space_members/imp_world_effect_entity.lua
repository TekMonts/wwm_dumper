-- ======================================================================
-- Module: hexm.client.entities.local.space_members.imp_world_effect_entity
-- Source: package.loaded
-- Type: table
-- Order: #1501
-- ======================================================================

-- Module type: table

SpaceMember: class {
  -- Metatable:
  --   __tostring: yes
  __fini_component__: function(arg1)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:23-26
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:15-17
  __module__: "hexm/client/entities/local/space_members/imp_world_effect_entity.lua"
  __post_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:19-21
  _real_clear_effect_tach_entity_effect: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:48-53
  _real_detach_effect_tach_entity: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:40-46
  ctor: function(...)  -- =[C]
  detach_effect_tach_entity: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:55-72
  get_effect_tach_entity: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:28-38
  new: function(...)  -- =[C]
  remove_effect_tach_entity: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_world_effect_entity.lua:74-80
}


-- End of hexm.client.entities.local.space_members.imp_world_effect_entity