-- ======================================================================
-- Module: hexm.client.entities.local.space_members.imp_shelter_map
-- Source: package.loaded
-- Type: table
-- Order: #745
-- ======================================================================

-- Module type: table

SpaceMember: class {
  -- Metatable:
  --   __tostring: yes
  __fini_component__: function(arg1)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:27-38
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:15-18
  __module__: "hexm/client/entities/local/space_members/imp_shelter_map.lua"
  __post_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:20-25
  __space_load_over_component__: function(arg1)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:40-42
  _require_shelter_map_callback: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:76-78
  _shelter_map_callback: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:44-50
  ctor: function(...)  -- =[C]
  new: function(...)  -- =[C]
  require_shelter_map_pop_flag: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:87-92
  require_shelter_map_push_flag: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:80-85
  shelter_map_pop_flag: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:64-74
  shelter_map_push_flag: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_shelter_map.lua:52-62
}


-- End of hexm.client.entities.local.space_members.imp_shelter_map