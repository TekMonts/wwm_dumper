-- ======================================================================
-- Module: hexm.client.entities.local.space_members.imp_building_client_bp
-- Source: package.loaded
-- Type: table
-- Order: #2084
-- ======================================================================

-- Module type: table

ImpBuildingClientBp: class {
  -- Metatable:
  --   __tostring: yes
  __fini_component__: function(arg1)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:21-24
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:12-15
  __module__: "hexm/client/entities/local/space_members/imp_building_client_bp.lua"
  __post_component__: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:17-19
  building_add_client_bp: function(arg1, arg2, arg3, arg4)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:30-38
  building_delete_client_bp: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:40-50
  building_get_chunk_client_bp_data: function(arg1, arg2)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:26-28
  building_update_client_bp: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:52-61
  building_update_client_bp_comps: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/space_members/imp_building_client_bp.lua:63-81
  ctor: function(...)  -- =[C]
  new: function(...)  -- =[C]
}


-- End of hexm.client.entities.local.space_members.imp_building_client_bp