-- ======================================================================
-- Module: hexm.client.entities.local.npc_members.imp_anim_suffix
-- Source: package.loaded
-- Type: table
-- Order: #274
-- ======================================================================

-- Module type: table

NpcMember: class {
  -- Metatable:
  --   __tostring: yes
  __enter_space_component__: function(arg1, arg2)  -- @hexm/client/entities/local/npc_members/imp_anim_suffix.lua:9-17
  __module__: "hexm/client/entities/local/npc_members/imp_anim_suffix.lua"
  __on_revived_component__: function(arg1)  -- @hexm/client/entities/local/npc_members/imp_anim_suffix.lua:19-21
}


-- End of hexm.client.entities.local.npc_members.imp_anim_suffix