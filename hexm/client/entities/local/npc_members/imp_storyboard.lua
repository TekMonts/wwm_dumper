-- ======================================================================
-- Module: hexm.client.entities.local.npc_members.imp_storyboard
-- Source: package.loaded
-- Type: table
-- Order: #19
-- ======================================================================

-- Module type: table

NpcMember: class {
  -- Metatable:
  --   __tostring: yes
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/npc_members/imp_storyboard.lua:14-21
  __module__: "hexm/client/entities/local/npc_members/imp_storyboard.lua"
  on_setup_storyboard_speed: function(arg1, arg2)  -- @hexm/client/entities/local/npc_members/imp_storyboard.lua:23-30
}


-- End of hexm.client.entities.local.npc_members.imp_storyboard