-- ======================================================================
-- Module: hexm.client.entities.local.npc_members.imp_behavior_event
-- Source: package.loaded
-- Type: table
-- Order: #13
-- ======================================================================

-- Module type: table

NpcMember: class {
  -- Metatable:
  --   __tostring: yes
  __module__: "hexm/client/entities/local/npc_members/imp_behavior_event.lua"
  trigger_behavior_anim_action_donation: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/npc_members/imp_behavior_event.lua:19-27
  trigger_behavior_anim_action_invite: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/npc_members/imp_behavior_event.lua:14-17
}


-- End of hexm.client.entities.local.npc_members.imp_behavior_event