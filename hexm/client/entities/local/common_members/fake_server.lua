-- ======================================================================
-- Module: hexm.client.entities.local.common_members.fake_server
-- Source: package.loaded
-- Type: table
-- Order: #1296
-- ======================================================================

-- Module type: table

FakeServer: class {
  -- Metatable:
  --   __tostring: yes
  __enter_space_component__: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:40-42
  __fini_component__: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:28-30
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:32-34
  __leave_space_component__: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:15-20
  __module__: "hexm/client/entities/local/common_members/fake_server.lua"
  __post_component__: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:36-38
  __pre_fini_component__: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:22-26
  _create_fake_server: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:85-97
  _create_fake_server_if_needed: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:58-62
  _destroy_fake_server: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:138-143
  _fake_on_local_entity_created: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:105-109
  _init_fake_server: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:99-103
  check_create_fake_server: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:44-46
  ctor: function(...)  -- =[C]
  dump_data_before_clear_fake: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:111-113
  get_fake_cls: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:48-56
  get_fake_server: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:64-71
  mode_fake_server_clear: function(arg1)  -- @hexm/client/entities/local/common_members/fake_server.lua:115-117
  mode_fake_server_recover: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:119-136
  new: function(...)  -- =[C]
  set_fake_server: function(arg1, arg2)  -- @hexm/client/entities/local/common_members/fake_server.lua:73-83
}


-- End of hexm.client.entities.local.common_members.fake_server