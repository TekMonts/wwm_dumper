-- ======================================================================
-- Module: hexm.client.entities.local.ai_avatar_members.imp_ai
-- Source: package.loaded
-- Type: table
-- Order: #3066
-- ======================================================================

-- Module type: table

AIAvatarMember: class {
  -- Metatable:
  --   __tostring: yes
  __check_component_valid__: "check_ai_component_valid"
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/ai_avatar_members/imp_ai.lua:15-19
  __module__: "hexm/client/entities/local/ai_avatar_members/imp_ai.lua"
  check_ai_component_valid: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_ai.lua:9-13
  need_ai: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_ai.lua:29-31
  on_sunshine_select_self: function(arg1, arg2, arg3)  -- @hexm/client/entities/local/ai_avatar_members/imp_ai.lua:33-52
  register_set_edit_entity_event: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_ai.lua:21-27
}


-- End of hexm.client.entities.local.ai_avatar_members.imp_ai