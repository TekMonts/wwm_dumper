-- ======================================================================
-- Module: hexm.client.entities.local.ai_avatar_members.imp_run
-- Source: package.loaded
-- Type: table
-- Order: #3051
-- ======================================================================

-- Module type: table

AIAvatarMember: class {
  -- Metatable:
  --   __tostring: yes
  __module__: "hexm/client/entities/local/ai_avatar_members/imp_run.lua"
  ctor: function(...)  -- =[C]
  enter_crouch: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_run.lua:13-20
  is_in_quick_run: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_run.lua:5-11
  leave_crouch: function(arg1)  -- @hexm/client/entities/local/ai_avatar_members/imp_run.lua:22-28
  new: function(...)  -- =[C]
}


-- End of hexm.client.entities.local.ai_avatar_members.imp_run