-- ======================================================================
-- Module: hexm.client.entities.local.billboard_entity_members.imp_sync_pos
-- Source: package.loaded
-- Type: table
-- Order: #1490
-- ======================================================================

-- Module type: table

BillboardEntityMember: class {
  -- Metatable:
  --   __tostring: yes
  __enter_space_component__: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:12-14
  __init_component__: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:6-10
  __leave_space_component__: function(arg1)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:16-18
  __module__: "hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua"
  _enable_sync_pos_tick: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:56-66
  _sync_pos: function(arg1)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:68-73
  ctor: function(...)  -- =[C]
  is_paused_sync_pos: function(arg1)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:41-43
  new: function(...)  -- =[C]
  pause_sync_pos: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:20-30
  resume_sync_pos: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:32-39
  set_sync_pos_interval: function(arg1, arg2)  -- @hexm/client/entities/local/billboard_entity_members/imp_sync_pos.lua:45-54
}


-- End of hexm.client.entities.local.billboard_entity_members.imp_sync_pos