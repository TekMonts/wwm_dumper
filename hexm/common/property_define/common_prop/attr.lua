-- ======================================================================
-- Module: hexm.common.property_define.common_prop.attr
-- Source: package.loaded
-- Type: table
-- Order: #4719
-- ======================================================================

-- Module type: table

Attr: class {
  -- Metatable:
  --   __tostring: yes
  __module__: "hexm/common/property_define/common_prop/attr.lua"
  __property_all__: instance {
    -- Metatable:
    --   __index: table
    --   __tostring: yes
    ABR_AVOID_PROB: 0.0
    ACR_PROB: 0.0
    ADD_ACR_PROB: 0.0
    ADD_ACR_RESIST_PROB: 0.0
    ADD_BASH_PROB: 0.0
    ADD_BASH_RESIST_PROB: 0.0
    ADD_CRI_PROB: 0.0
    ADD_CRI_RESIST_PROB: 0.0
    ALL_ONELEVEL_ATTR: 0.0
    BASH_PROB: 0.0
    BASH_UP: 0.0
    BASH_UP_RDC: 0.0
    CRI_PROB: 0.0
    CS_ITS: 0.0
    DAMAGE_REDUCE: 0.0
    DIRECT_BASH_PROB: 0.0
    DIRECT_CRI_PROB: 0.0
    GP_BEHIT_DMG_IND: 0.0
    GP_DIR: 0.0
    GP_DMG_IND: 0.0
    GP_REAL_DMG_IND: 0.0
    HEAL_CRI_UP: 0.0
    HP: -1.0
    HP_MAX: 0.0
    JUDGE_ATTR_CORRECT_PARA: 0.0
    LIFE_ALT_ATK: 0.0
    LIFE_ALT_DEF: 0.0
    LIFE_ALT_HP: 0.0
    LIFE_ALT_HP_MAX: 0.0
    LIFE_COLD_ATK: 0.0
    LIFE_COLD_DEF: 0.0
    LIFE_COLD_HP: 0.0
    LIFE_COLD_HP_MAX: 0.0
    LIFE_HOT_ATK: 0.0
    LIFE_HOT_DEF: 0.0
    LIFE_HOT_HP: 0.0
    LIFE_HOT_HP_MAX: 0.0
    LIFE_MIND_ATK: 0.0
    LIFE_MIND_DEF: 0.0
    LIFE_MIND_HP: 0.0
    LIFE_MIND_HP_MAX: 0.0
    LIFE_SWIM_ATK: 0.0
    LIFE_SWIM_DEF: 0.0
    LIFE_SWIM_HP: 0.0
    LIFE_SWIM_HP_MAX: 0.0
    LIFE_TOXIC_ATK: 0.0
    LIFE_TOXIC_DEF: 0.0
    LIFE_TOXIC_HP: 0.0
    LIFE_TOXIC_HP_MAX: 0.0
    LIFE_WET_ATK: 0.0
    LIFE_WET_DEF: 0.0
    LIFE_WET_HP: 0.0
    LIFE_WET_HP_MAX: 0.0
    MAX_N_ATK: 0.0
    MAX_PRO_ATK_A: 0.0
    MAX_PRO_ATK_B: 0.0
    MAX_PRO_ATK_C: 0.0
    MAX_PRO_ATK_D: 0.0
    MAX_PRO_ATK_E: 0.0
    MAX_W_ATK: 0.0
    MIN_N_ATK: 0.0
    MIN_PRO_ATK_A: 0.0
    MIN_PRO_ATK_B: 0.0
    MIN_PRO_ATK_C: 0.0
    MIN_PRO_ATK_D: 0.0
    MIN_PRO_ATK_E: 0.0
    MIN_W_ATK: 0.0
    N_ATK: 0.0
    N_ATK_CRI_UP: 0.0
    N_ATK_CRI_UP_RDC: 0.0
    N_ATK_DEF_MAX: 0.0
    N_ATK_IGR: 0.0
    N_ATK_IGR_RDC: 0.0
    N_ATK_PEN: 0.0
    N_ATK_PEN_MAX: 0.0
    N_ATK_PEN_RDC: 0.0
    N_ATK_SCALE: 0.0
    N_DEF: 0.0
    N_DEF_SCALE: 0.0
    N_HEAL: 0.0
    N_HEAL_RDC_SCALE: 0.0
    N_HEAL_SCALE: 0.0
    PRO_ATK_A_SCALE: 0.0
    PRO_ATK_B_SCALE: 0.0
    PRO_ATK_CRI_UP: 0.0
    PRO_ATK_CRI_UP_RDC: 0.0
    PRO_ATK_C_SCALE: 0.0
    PRO_ATK_D_SCALE: 0.0
    PRO_ATK_E_SCALE: 0.0
    PRO_ATK_H: 0.0
    PRO_DEF_A: 0.0
    PRO_DEF_A_SCALE: 0.0
    PRO_DEF_B: 0.0
    PRO_DEF_B_SCALE: 0.0
    PRO_DEF_C: 0.0
    PRO_DEF_C_SCALE: 0.0
    PRO_DEF_D: 0.0
    PRO_DEF_D_SCALE: 0.0
    PRO_DEF_E: 0.0
    PRO_DEF_E_SCALE: 0.0
    PRO_HEAL_SCALE: 0.0
    PRO_PEN_A: 0.0
    PRO_PEN_B: 0.0
    PRO_PEN_C: 0.0
    PRO_PEN_D: 0.0
    PRO_PEN_E: 0.0
    PRO_PEN_H: 0.0
    QIJUE_DAMAGE_SCALE: 0.0
    QIJUE_REDUCE: 0.0
    SKILL_JIEFU_HD_ADD: 0.0
    SKILL_JIJU_LX_ADD: 0.0
    SKILL_MINGCHUAN_ZLJ_ADD: 0.0
    STATE_LEVEL_EXPLORE: 1
    STATE_LEVEL_KUNGFU: 1
    STATE_LEVEL_TRADE: 1
    TOTAL_PRO_ATK_MAX: 0.0
    TOTAL_PRO_ATK_MIN: 0.0
    WEAK_DMGUP_RATE: 0.0
    W_ATK: 0.0
    W_ATK_CRI_UP: 0.0
    W_ATK_CRI_UP_RDC: 0.0
    W_ATK_IGR: 0.0
    W_ATK_IGR_RDC: 0.0
    W_ATK_PEN: 0.0
    W_ATK_PEN_RDC: 0.0
    W_ATK_SCALE: 0.0
    W_DEF: 0.0
    W_DEF_SCALE: 0.0
    W_HEAL_RDC_SCALE: 0.0
    W_HEAL_SCALE: 0.0
    XIUWEI_EXPLORE: 0
    XIUWEI_KUNGFU: 0
    XIUWEI_TRADE: 0
    XUEWEI_ATK: 0
    XUEWEI_A_DEF: 0
    XUEWEI_B_DEF: 0
    XUEWEI_C_DEF: 0
    XUEWEI_D_DEF: 0
    XUEWEI_E_DEF: 0
    ZHENQI_DAMAGE: 0.0
    ZHENQI_DEF: 0.0
    ZHENQI_REPLY: 0.0
    __packed_data__: ""
    cr_refreshing: class {
      -- Metatable:
      --   __tostring: yes
      __module__: "hexm/common/property_define/common_prop/attr_base.lua"
      __property_all__: <dict>
      __property_flag__: <dict>
      __property_index__: <instance>
      _initProperty: function(arg1, arg2)  -- @engine/common/classutils.lua:310-330
    }
    dynamic_attr: class {
      -- Metatable:
      --   __tostring: yes
      __module__: "hexm/common/property_define/common_prop/attr_base.lua"
      __property_all__: <dict>
      __property_flag__: <dict>
      __property_index__: <instance>
      _initProperty: function(arg1, arg2)  -- @engine/common/classutils.lua:310-330
    }
    own_dynamic_attr: <circular>
    res_recover_ts: class {
      -- Metatable:
      --   __tostring: yes
      __module__: "hexm/common/property_define/common_prop/attr_base.lua"
      __property_all__: <dict>
      __property_flag__: <dict>
      __property_index__: <instance>
      _initProperty: function(arg1, arg2)  -- @engine/common/classutils.lua:310-330
    }
    resource_empty: class {
      -- Metatable:
      --   __tostring: yes
      IS_CUSTOM_TYPE: true
      __len: nil
      __metaclass__: function(arg1)  -- @engine/common/classutils.lua:592-594
      __module__: "engine/common/classutils.lua"
      __property_all__: <dict>
      __property_flag__: <dict>
      __property_index__: <instance>
      _initProperty: function(arg1, arg2)  -- @engine/common/classutils.lua:286-308
      ctor: function(arg1, arg2)  -- @engine/common/classutils.lua:612-621
      items: function(arg1)  -- @engine/common/classutils.lua:655-661
      keys: function(arg1)  -- @engine/common/classutils.lua:639-645
      on_assign: function(arg1)  -- @engine/common/classutils.lua:632-633
      on_clear: function(arg1)  -- @engine/common/classutils.lua:626-627
      on_init: function(arg1, arg2)  -- @engine/common/classutils.lua:623-624
      on_setattr: function(arg1, arg2, arg3, arg4)  -- @engine/common/classutils.lua:636-637
      on_update: function(arg1, arg2)  -- @engine/common/classutils.lua:629-630
      setdefault: function(arg1, arg2, arg3)  -- @engine/common/classutils.lua:663-670
      to_valid_dict: function(arg1)  -- @engine/common/classutils.lua:672-709
      values: function(arg1)  -- @engine/common/classutils.lua:647-653
    }
    resource_id_map: <circular>
    salt: 0
  }
  __property_flag__: instance {
    -- Metatable:
    --   __index: table
    --   __tostring: yes
    ABR_AVOID_PROB: 2
    ACR_PROB: 2
    ADD_ACR_PROB: 2
    ADD_ACR_RESIST_PROB: 2
    ADD_BASH_PROB: 2
    ADD_BASH_RESIST_PROB: 2
    ADD_CRI_PROB: 2
    ADD_CRI_RESIST_PROB: 2
    ALL_ONELEVEL_ATTR: 2
    BASH_PROB: 2
    BASH_UP: 2
    BASH_UP_RDC: 2
    CRI_PROB: 2
    CS_ITS: 2
    DAMAGE_REDUCE: 2
    DIRECT_BASH_PROB: 2
    DIRECT_CRI_PROB: 2
    GP_BEHIT_DMG_IND: 2
    GP_DIR: 2
    GP_DMG_IND: 2
    GP_REAL_DMG_IND: 2
    HEAL_CRI_UP: 2
    HP: 12
    HP_MAX: 12
    JUDGE_ATTR_CORRECT_PARA: 2
    LIFE_ALT_ATK: 1
    LIFE_ALT_DEF: 1
    LIFE_ALT_HP: 1
    LIFE_ALT_HP_MAX: 1
    LIFE_COLD_ATK: 1
    LIFE_COLD_DEF: 2
    LIFE_COLD_HP: 1
    LIFE_COLD_HP_MAX: 1
    LIFE_HOT_ATK: 1
    LIFE_HOT_DEF: 2
    LIFE_HOT_HP: 1
    LIFE_HOT_HP_MAX: 1
    LIFE_MIND_ATK: 1
    LIFE_MIND_DEF: 2
    LIFE_MIND_HP: 1
    LIFE_MIND_HP_MAX: 1
    LIFE_SWIM_ATK: 1
    LIFE_SWIM_DEF: 1
    LIFE_SWIM_HP: 1
    LIFE_SWIM_HP_MAX: 1
    LIFE_TOXIC_ATK: 1
    LIFE_TOXIC_DEF: 2
    LIFE_TOXIC_HP: 1
    LIFE_TOXIC_HP_MAX: 1
    LIFE_WET_ATK: 1
    LIFE_WET_DEF: 1
    LIFE_WET_HP: 1
    LIFE_WET_HP_MAX: 1
    MAX_N_ATK: 2
    MAX_PRO_ATK_A: 2
    MAX_PRO_ATK_B: 2
    MAX_PRO_ATK_C: 2
    MAX_PRO_ATK_D: 2
    MAX_PRO_ATK_E: 2
    MAX_W_ATK: 2
    MIN_N_ATK: 2
    MIN_PRO_ATK_A: 2
    MIN_PRO_ATK_B: 2
    MIN_PRO_ATK_C: 2
    MIN_PRO_ATK_D: 2
    MIN_PRO_ATK_E: 2
    MIN_W_ATK: 2
    N_ATK: 2
    N_ATK_CRI_UP: 2
    N_ATK_CRI_UP_RDC: 2
    N_ATK_DEF_MAX: 2
    N_ATK_IGR: 2
    N_ATK_IGR_RDC: 2
    N_ATK_PEN: 2
    N_ATK_PEN_MAX: 2
    N_ATK_PEN_RDC: 2
    N_ATK_SCALE: 2
    N_DEF: 2
    N_DEF_SCALE: 2
    N_HEAL: 2
    N_HEAL_RDC_SCALE: 2
    N_HEAL_SCALE: 2
    PRO_ATK_A_SCALE: 2
    PRO_ATK_B_SCALE: 2
    PRO_ATK_CRI_UP: 2
    PRO_ATK_CRI_UP_RDC: 2
    PRO_ATK_C_SCALE: 2
    PRO_ATK_D_SCALE: 2
    PRO_ATK_E_SCALE: 2
    PRO_ATK_H: 2
    PRO_DEF_A: 2
    PRO_DEF_A_SCALE: 2
    PRO_DEF_B: 2
    PRO_DEF_B_SCALE: 2
    PRO_DEF_C: 2
    PRO_DEF_C_SCALE: 2
    PRO_DEF_D: 2
    PRO_DEF_D_SCALE: 2
    PRO_DEF_E: 2
    PRO_DEF_E_SCALE: 2
    PRO_HEAL_SCALE: 2
    PRO_PEN_A: 2
    PRO_PEN_B: 2
    PRO_PEN_C: 2
    PRO_PEN_D: 2
    PRO_PEN_E: 2
    PRO_PEN_H: 2
    QIJUE_DAMAGE_SCALE: 2
    QIJUE_REDUCE: 2
    SKILL_JIEFU_HD_ADD: 2
    SKILL_JIJU_LX_ADD: 2
    SKILL_MINGCHUAN_ZLJ_ADD: 2
    STATE_LEVEL_EXPLORE: 1
    STATE_LEVEL_KUNGFU: 1
    STATE_LEVEL_TRADE: 1
    TOTAL_PRO_ATK_MAX: 2
    TOTAL_PRO_ATK_MIN: 2
    WEAK_DMGUP_RATE: 2
    W_ATK: 2
    W_ATK_CRI_UP: 2
    W_ATK_CRI_UP_RDC: 2
    W_ATK_IGR: 2
    W_ATK_IGR_RDC: 2
    W_ATK_PEN: 2
    W_ATK_PEN_RDC: 2
    W_ATK_SCALE: 2
    W_DEF: 2
    W_DEF_SCALE: 2
    W_HEAL_RDC_SCALE: 2
    W_HEAL_SCALE: 2
    XIUWEI_EXPLORE: 1
    XIUWEI_KUNGFU: 2
    XIUWEI_TRADE: 1
    XUEWEI_ATK: 4
    XUEWEI_A_DEF: 4
    XUEWEI_B_DEF: 4
    XUEWEI_C_DEF: 4
    XUEWEI_D_DEF: 4
    XUEWEI_E_DEF: 4
    ZHENQI_DAMAGE: 2
    ZHENQI_DEF: 2
    ZHENQI_REPLY: 2
    __packed_data__: 1
    cr_refreshing: 4
    dynamic_attr: 12
    own_dynamic_attr: 74
    res_recover_ts: 2
    resource_empty: 2
    resource_id_map: 12
    salt: 4
  }
  __property_index__: <instance>
  _initProperty: function(arg1, arg2)  -- @engine/common/classutils.lua:310-330
}


-- End of hexm.common.property_define.common_prop.attr